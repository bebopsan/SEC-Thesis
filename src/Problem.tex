...
In this project we proposed to develop a simulation software package that allows analysis of electromagnetic (EM) wave propagation, in the context of Photonic Crystals (PCs)\remove[SEC]{defects modeling and design}.
...
\section{Objectives }
\remove[SEC]{The following were the objectives as stated in the project proposal document:}
\subsection{General objective\remove[SEC]{s}}
%\remove[SEC]{
%\begin{itemize}
...
%\end{itemize}}
\add[SEC]{Build software capable of simulating electromagnetic field propagation in Photonic Crystals.}\note[SEC]{Cuando mencionaba integración no me refería a simulaciones multifísicas sino a que la plataforma tenga la flexibilidad para simular distintos fenomenos (no simulatneos)}

% Objectives must be S.M.A.R.T:
% Specific, Measurable, Achievable, Relevant and Time-bound.%

\subsection{Specific objectives}
\note[SEC]{Se removió todo el objetivo específico 1 porque adquirir la teoría no es un objetivo medible objetivamente. Con esto se elimina también la sección que hace mención a aplicaciones tecnológicas. Cabe mencionar que en el documento original las aplicaciones tecnológicas se comentan en este objetivo como algo a entender y no a desarrollar; es decir, que yo entienda cómo se pueden usar los PCs para aplicaciones tecnológicas y eso si se hizo.} 
\begin{enumerate}
...
\item Implement the routines that form the platform and perform tests \change[SEC]{that relate to technological applications studied in the appropriation stage}{that validate its performance.} \add[SEC]{Such tests include:} 
	\begin{enumerate}
...
The next frontier in the context of material science\remove[SEC]{,} relies in a level of understanding that allows us not only to use and transform materials, but to design them. This is the field of ``\textit{metamaterials}'', a field in which by means of manipulating the microscopical structure and geometry of a crystal-like solid, we can obtain macroscopic properties that are otherwise impossible to find in nature \cite{Engheta2006, Capoliono2009}. One of the sub domains of the metamaterials frontier is that of media capable of molding and manipulating electromagnetic waves. The ability to harness light has the prospect to support a new technological revolution of a scale similar to that of transistor revolution \cite{Joannopoulos2008}.
