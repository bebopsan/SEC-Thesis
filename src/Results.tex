...
A rectangular region \add{filled with a perfect dielectric and with perfect conductor upper and lower plates} was meshed and Dirichlet boundary condition were applied.  Separation $d$ is taken unitary and the result is shown in figure \ref{fig:capacitor}. \add[SEC]{Given that in the implementation units have been normalized we can assign arbitrary values of field to the solution. It can be assumed that the constant value of 12 corresponds to $V/m$}
...
Where $\int_V \rho dV = q$ is the total charge of the \change[SEC]{particle}{cylinder}. If that charge is distributed uniformly, then there are no preferred directions and the electric field must be radial \cite{Cheng1993}, and have constant intensity over spherical shells. So for an arbitrary 
...
\caption{Whole simulation of Electric field due to a charged cylinder. a) Numerical result for the $x$ component of the field. b) $y$ component of the solution right. c) Mesh and error calculated between analytic formula and results. Units are arbitrary electric field units such as $V/m$.}
