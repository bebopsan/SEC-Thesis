...
\end{quote}
We used gmsh as both a Computer Asisted Design (CAD) tool and mesh generator for the modeling of simulation domains used in most part of the results presented in \ref{ch:results}. It could be said that Gmsh is the main pre-processing tool used by PeyeQM.
...
\caption{This diagram shows the classes defined in the module gmshlibrary.py. Classes that are called by a method are connected with lines that end with a black diamonds, inheritance is shown with a white arrow, and classes that are attributes of other classes are connected by white diamonds.}
